...
public class EmployeeService {
    ...
    public List<LeaveReqOfEmpResDto> getLeaveRequest(String loggedInEmail,RequestStatus status, LeaveType leaveType) {
        log.info("Fetching leave requests for employee email: {}", loggedInEmail);

        Employee employee = employeeRepository.findByUser_UserEmail(loggedInEmail);
        List<LeaveRequest> list = leaveRequestService.getLeaveRequest(employee.getId(),status, leaveType);
    ...
    public PageLeaveReqOfEmpResDto getLeaveRequestPage(String loggedInEmail,RequestStatus status, LeaveType leaveType,int page,int size) {
        log.info("Fetching leave requests for employee email: {}", loggedInEmail);

        Pageable pageable =  PageRequest.of(page, size);

        Employee employee = employeeRepository.findByUser_UserEmail(loggedInEmail);
        Page<LeaveRequest> list = leaveRequestService.getLeaveRequestPage(employee.getId(),status, leaveType,pageable);
        log.info("Found {} leave requests for employee ID: {}",  employee.getId());
        List<LeaveReqOfEmpResDto> data = list.stream()
                .map(Dto-> new LeaveReqOfEmpResDto(
                        Dto.getId(),
                        Dto.getLeaveType(),
                        Dto.getStatus(),
        ...
        return new PageLeaveReqOfEmpResDto(data,list.getTotalElements(),list.getTotalPages());
    ...
    public void submitEmployeeDetails(EmployeeProfileSubmitDto profileDto, String loggedInEmail) {
        ...
        //2.5 update the status of user
        userService.updateEmployeeUserProfile(user);
    ...
    public EmployeeProfileResDto getProfile(String loggedInEmail) {
        ...
        return new EmployeeProfileResDto(
                employee.getId(),
                ...
                employee.getStatus(),
                managerName,
                employee.getUser().getUserEmail(),
                employee.getUser().getRole().toString()
        );
    }
}
