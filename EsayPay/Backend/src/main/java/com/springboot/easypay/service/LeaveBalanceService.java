...
import com.springboot.easypay.mapper.LeaveBalanceMapper;
...
public class LeaveBalanceService {
    ...
    public List<LeaveBalanceStatResDto> getLeaveBalanceStat(String loggedInEmail) {
        log.info("Fetching leave statistics for: {}", loggedInEmail);

        Employee employee = employeeRepository.findByUser_UserEmail(loggedInEmail);
        List<LeaveBalance> list =  leaveBalanceRepository.findByEmployeeId(employee.getId());

        log.info("Retrieved {} leave types for Employee ID: {}", list.size(), employee.getId());
        return list.stream().map(LeaveBalanceMapper:: mapToDto).toList();
    }
}
